    ...
    public class Problem018 : Problem
    {
        ...
        public Problem018()
        {
            Source = AppConfigParameters
                .Keys[ProjectEulerConstants.Problem018Key]
                .Collection[ProjectEulerConstants.Problem018SourceKey];
        }

        public Problem018(string source)
        {
            Source = source;
        }
        ...
        public override dynamic Solve()
        {
            var flatFileDataManager = new FlatFileDataManager(
                BuiltInType.Integer,
                CollectionType.ArrayJagged,
                DelimiterType.Space,
                Source);
            var triangle = flatFileDataManager.Input;
            for (var i = triangle.GetLength(0) - 2; i >= 0; i--)
            {
                for (var j = 0; j <= i; j++)
                {
                    triangle[i][j] +=
        ...
        {
            ResultMessage =
                "The maximum total from top to bottom of the provided triangle [" +
                Source +
                "] is [" +
                _maxPathSum +
                "].";
